...
Jweight = 0;%ÈšÖµ³Í·£
Jsparse = 0;%Ï¡ÊèÐÔ³Í·£
[n m] = size(data);%mÎªÑù±ŸµÄžöÊý£¬nÎªÑù±ŸµÄÌØÕ÷Êý

%Ç°ÏòËã·šŒÆËãž÷ÉñŸ­ÍøÂçœÚµãµÄÏßÐÔ×éºÏÖµºÍactiveÖµ
z2 = W1*data+repmat(b1,1,m);%×¢ÒâÕâÀïÒ»¶šÒªœ«b1ÏòÁ¿žŽÖÆÀ©Õ¹³ÉmÁÐµÄŸØÕó
a2 = sigmoid(z2);
...
%ŒÆËãÏ¡ÊÍÐÔ¹æÔòÏî
rho = (1/m).*sum(a2,2);%Çó³öµÚÒ»žöÒþº¬²ãµÄÆœŸùÖµÏòÁ¿
...
%·ŽÏòËã·šÇó³öÃ¿žöœÚµãµÄÎó²îÖµ
d3 = -(data-a3).*sigmoidInv(z3);
sterm = beta*(-sparsityParam./rho+(1-sparsityParam)./(1-rho));%ÒòÎªŒÓÈëÁËÏ¡Êè¹æÔòÏî£¬ËùÒÔ
                                                             %ŒÆËãÆ«µŒÊ±ÐèÒªÒýÈëžÃÏî
...
b1grad = (1/m)*b1grad;%×¢ÒâbµÄÆ«µŒÊÇÒ»žöÏòÁ¿£¬ËùÒÔÕâÀïÓŠžÃ°ÑÃ¿Ò»ÐÐµÄÖµÀÛŒÓÆðÀŽ
