function run3DID_AllVectorsChooseNegRand_aaIndex_test_load_DL_cross( ...
                                            file_ddis, FisherMode, SVMMode, Kernel, choseNegRatio, choseNegRatioTest)
addpath(genpath('/home/michael/Dropbox/Project/Contact_maps/'));
diary off;
dateFormatted = upper(date);
dateFormatted = dateFormatted(regexp(upper(date), '[A-Z0-9]'));
logfile_name = ['log_cross_run3DID_AllVectorsChooseNegRand_aaIndex_test_load_DL_' ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% get examples.
MatrixFile=['/home/michael/Documents/Contact_Matrix_Project/Family_' FisherMode '.mat' ];
if exist(MatrixFile, 'file')
    calOpt=0; % controls weather to calculate the data vectors.
    load(MatrixFile);
else
    calOpt=1;
end
...
if calOpt
    ...
    save(MatrixFile, 'FamilyPO', 'FamilyNE');
end


%matlabpool open local 2 %number of parallel machine max=8
for ddi_ctr = 1:length(cell_ddis)
    ...
    try
        ...
        [aucBL_Array, blSens_Array, blSpec_Array, ...
                    ...
                    trainAccuracy_Array, trainRecall_Array, trainPrecision_Array, train_f_measure_Array]  = runDDIAllVectorsChooseNegRand_aaIndex_test_load_DL_cross ...
