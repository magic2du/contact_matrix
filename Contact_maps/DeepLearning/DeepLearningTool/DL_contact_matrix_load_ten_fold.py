...
logname = 'DL_contact_matrix_' + current_date + '.log'
...
class DDI_family_base(object):
    ...
    #def __init__(self, ddi, Vectors_Fishers_aaIndex_raw_folder = '/big/du/Protein_Protein_Interaction_Project/Contact_Matrix_Project/Vectors_Fishers_aaIndex_raw_2014/'):
...
def saveAsCsv(predicted_score, fname, *arguments): #new
    newfile = False
...
class LOO_out_performance_for_one_ddi(object):
        ...
        def get_LOO_perfermance(self, fisher_mode, settings = None):
            ...
            report_name = filename + '_' + '_'.join(map(str, hidden_layers_sizes)) +                             '_' + str(pretrain_lr) + '_' + str(finetune_lr) + '_' + str(reduce_ratio)+current_date
            saveAsCsv(predicted_score, report_name, analysis_scr)
...
def ten_fold_crossvalid_performance_for_all(ddis):
    for ddi in ddis:
        try:
            ...
            print "error in ddi: %s" % ddi
            logger.info('error in ddi: %s' % ddi)
...
class Ten_fold_crossvalid_performance_for_one_ddi(object):
        ...
        def get_ten_fold_crossvalid_perfermance(self, fisher_mode, settings = None):
            ...
            report_name = filename + '_' + '_test10fold'.join(map(str, hidden_layers_sizes)) +                             '_' + str(pretrain_lr) + '_' + str(finetune_lr) + '_' + str(reduce_ratio)+ '_' + current_date
            saveAsCsv(predicted_score, report_name, analysis_scr)


# In[39]:

ten_fold_crossvalid_performance_for_all(ddis[:1])
