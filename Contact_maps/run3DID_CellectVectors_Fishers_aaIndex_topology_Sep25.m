function run3DID_CellectVectors_Fishers_aaIndex_topology_Sep25(file_ddis)
...
logfile_name = ['log_run3DID_CellectVectors_Fishers_aaIndex_topology_Sep25' ...
             dateFormatted '.txt'];
...
for ddi_ctr = 1:length(cell_ddis)
    ...
    try
        ...
        runDDI_CellectVectors_Fishers_aaIndex_topology_Sep25(ddiName, outputPath);
...
cd(currDir);
save(['successedDDI_topology_' file_ddis '_shorter_than150.txt'], 'successDDI', '-ascii');
c = clock;
time = [num2str(c(1), '%0.0d') '-' num2str(c(2), '%0.0d') '-' ...
        num2str(c(3), '%0.0d') ' ' num2str(c(4), '%0.0d') ':' ...
        num2str(c(5), '%0.0d')];
