...
settings['DL_new_contraction'] = 1
settings['DL_sparsity'] = 1
settings['DL_weight_decay'] = 1
settings['DL_Drop_out'] = 1

settings['finetune_lr'] = 0.1
settings['batch_size'] = 100
settings['pretraining_interations'] = 20003
...
logname = 'log_DL_handwritten_classification' + current_date + '.log'
...
########################################
                if settings.has_key('DL_new_contraction') and settings['DL_new_contraction']:
                    print 'DL_new_contraction'                    
                    cfg = settings.copy()
                    cfg['contraction_level'] = settings['contraction_level'] if settings.has_key('contraction_level') and (settings['contraction_level'] is not None) else 0
                    train_x = x_train_minmax; train_y = y_train_minmax                    
                    cfg['n_ins'] = train_x.shape[1]
                    sdaf = Sda_factory(cfg)
                    sda = sdaf.sda.pretraining(train_x = train_x)
                    ...
                    isTest = False #new
                    training_predicted = sdaf.dnn.predict.predict(x_train_minmax)
                    y_train = y_train_minmax                                       
                    analysis_scr.append(( subset_no, fisher_mode, 'DL_new_contraction', isTest) + tuple(performance_score(y_train, training_predicted, with_auc_score).values()))
                    
                    isTest = True #new
                    y_test = test_y
                    test_predicted = sdaf.dnn.predict.predict(x_test_minmax)
                    analysis_scr.append(( subset_no, fisher_mode, 'DL_new_contraction', isTest) + tuple(performance_score(y_test, test_predicted, with_auc_score).values()))            
               
                if settings['DL_new_sparsity'] == 1:
                    print 'DL_S_new_sparsity'
                    cfg = settings.copy()
                    cfg['sparsity'] = 0.01
                    cfg['sparsity_weight'] = 0.01
                    ...
                    training_predicted = sdaf.dnn.predict(x_train_minmax)
                    y_train = y_train_minmax                                       
                    analysis_scr.append(( subset_no, fisher_mode, 'DL_new_sparsity', isTest) + tuple(performance_score(y_train, training_predicted, with_auc_score).values()))
                    
                    isTest = True #new
                    y_test = test_y
                    test_predicted = sdaf.dnn.predict(x_test_minmax)
                    analysis_scr.append(( subset_no, fisher_mode, 'DL_new_sparsity', isTest) + tuple(performance_score(y_test, test_predicted, with_auc_score).values()))            
               
                if settings['DL_new_weight_decay'] == 2:
                    cfg = settings.copy()
                    cfg['l2_reg'] = settings['l2_reg'] if settings.has_key('l2_reg') and (settings['l2_reg'] is not None) else 0
                    print 'l2_reg'
                    train_x = x_train_minmax; train_y = y_train_minmax                    
                    cfg['n_ins'] = train_x.shape[1]
                    sdaf = Sda_factory(cfg)
                ...
                if settings['DL_new_Drop_out'] == 1:
                    
                    cfg = settings.copy()
                    cfg['dropout_factor'] = settings['dropout_factor'] if settings.has_key('dropout_factor') and (settings['dropout_factor'] is not None) else 0
                    print 'DL_new_Drop_out'
                    cfg['n_ins'] = train_x.shape[1]
                    sdaf = Sda_factory(cfg)
                    sda = sdaf.sda.pretraining(train_x = train_x)
                    sdaf.dnn.finetuning((x_train_minmax,  y_train_minmax),(x_validation_minmax, y_validation_minmax))    
                    ...
                    training_predicted = sdaf.dnn.predict(x_train_minmax)
                    y_train = y_train_minmax                                       
                    analysis_scr.append(( subset_no, fisher_mode, 'DL_new_Drop_out', isTest) + tuple(performance_score(y_train, training_predicted, with_auc_score).values()))
                    
                    isTest = True #new
                    y_test = test_y
                    test_predicted = sdaf.dnn.predict(x_test_minmax)
                    analysis_scr.append(( subset_no, fisher_mode, 'DL_new_Drop_out', isTest) + tuple(performance_score(y_test, test_predicted, with_auc_score).values()))            
