...
import numpy as np
# dump the un-normalized aaIndex in to a mat. struct as aaIndex
...
def dumpFeatureVectureMAT(): # dump the un-normalized aaIndex in to a mat. struct as aaIndex
    aaIndex={}
    aaMatrix=[]
    for aa in aaList:
...
#    mean, std, newaaMatrix=normorlizeList(aaMatrix)
    i=0
    for aa in aaList:
        aaIndex[aa]=np.array(aaMatrix[i,:])
        i+=1
#    print aaIndex['C']
    sio.savemat('aaIndex_withoutNormalization.mat', {'aaIndex': aaIndex})
