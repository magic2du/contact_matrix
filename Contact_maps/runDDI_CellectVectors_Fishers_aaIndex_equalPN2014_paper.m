function runDDI_CellectVectors_Fishers_aaIndex_equalPN2014_paper(ddiName, outputPath)
...
load(ddiStructFile);
allPairs=[1: length(ddi_str_array)];
% Save the pairs to run as a file
save([outputPath 'allPairs.txt'], 'allPairs', '-ascii');
pairsToRun= allPairs;
...
for pairCtr = 1:length(pairsToRun)
    ...
    [TrainingV, numPos, numNeg] = runSeqPair_CellectVectors_Fishers_aaIndex_equalPN2014_paper( ...
        ddiName, testPair);
	filename=[outputPath 'F0_20_F1_20_Sliding_17_11_F0_20_F1_20_Sliding_17_11_ouput_' num2str(testPair) '.txt']
