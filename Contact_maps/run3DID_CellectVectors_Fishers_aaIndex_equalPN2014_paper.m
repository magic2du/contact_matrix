function run3DID_CellectVectors_Fishers_aaIndex_equalPN2014_paper(file_ddis)
...
logfile_name = ['log_run3DID_CellectVectors_Fishers_aaIndex_All_2014_paper' ...
             dateFormatted '.txt'];
...
%for ddi_ctr = 1:length(cell_ddis)
    ...
    try
        ...
        outputPath=['/big/du/Protein_Protein_Interaction_Project/Contact_Matrix_Project/Vectors_Fishers_aaIndex_raw_2014_paper/' ddiName '/'] ;
		if ~exist(outputPath, 'dir')
			mkdir(outputPath);
		end
        runDDI_CellectVectors_Fishers_aaIndex_equalPN2014_paper(ddiName, outputPath);
        diary off;
	diary on;
