...
def getListForFamily(ddi):
	...
	summaryFileName=Folder+ddi+'/F1Vector.txt'
	try:
...
def getListForTopology(ddi):
	...
	for topology in topologyList:
		summaryFileName=Folder+ddi+'/'+topology+'/F1Vector.txt'
		...
		try:	
			...
			print 'This ddi '+ddi+topology+'does not have F1Vector.txt'
			return False
	return data
#start
todoList=sys.argv[1]
...
print 'The average F1 of the %d ddis at family level is %f' % (numberOfSuccessDDI, numpy.average(listTotalFamily))
print 'The standard deviation  of F1 at family level is %f' % numpy.std(listTotalFamily)
...
print 'The average F1 of the %d ddis at topology level is %f' % (numberOfSuccessDDI, numpy.average(listTotalTopology))
print 'The standard deviation  of F1 at topology level is %f' % numpy.std(listTotalTopology)
