...
def getListForFamily(ddi):
	...
	Folder='/home/du/Protein_Protein_Interaction_Project/3did_15OCT2010/dom_dom_ints/'
	summaryFileName=Folder+ddi+'/FisherM0MATLAB.summary'
	try:
		...
		if average<0.9 and len(aucList)>9:#if the average AUC less than 0.9 save this ddi
			listFile='listOfFamiliesWithAUC_LessThan0.9.txt'
			data_file = open(listFile,"a")
			data_file.write('%s %f %d\n' % (ddi,average,len(aucList)))
			data_file.close()
		return data
	except:
...
def getListForTopology(ddi):
	...
	for topology in topologyList:
		summaryFileName=Folder+ddi+'/'+topology+'/FisherM0MATLAB.summary'
		...
		try:	
			...
			print 'This ddi '+ddi+topology+'does not have FisherM0MATLAB.summary'
			return False
	return data
#start
todoList=sys.argv[1]
...
for item in listTotalFamily:#print average
	[auc,pvalue]=item
	sumOfAUCOfFamily+=auc
average=float(sumOfAUCOfFamily)/len(listTotalFamily)
print 'The average AUC of the %d ddis at family level is %f' % (len(ddis),average)
...
for item in listTotalTopology:
	[auc,pvalue]=item
	sumOfAUCOfTopology+=auc
average=float(sumOfAUCOfTopology)/len(listTotalTopology)
#print listTotalTopology
print 'The average AUC of the %d ddis at topology level is %f' % (len(ddis),average)
