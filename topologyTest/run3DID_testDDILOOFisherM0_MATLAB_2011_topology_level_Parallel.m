function run3DID_testDDILOOFisherM0_MATLAB_2011_topology_level_Parallel(file_ddis)

% logfile.
diary off;
dateFormatted = upper(date);
dateFormatted = dateFormatted(regexp(upper(date), '[A-Z0-9]'));
logfile_name = ['log_testDDILOOFisherM0_MATLAB_' dateFormatted '.txt'];
...
time = [num2str(c(1), '%0.0d') '-' num2str(c(2), '%0.0d') '-' ...
        ...
        num2str(c(5), '%0.0d')];
logline = [time ' STARTED, testDDILOOFisherM0MATLAB.\n'];
fprintf(logline);
...
parfor ddi_ctr = 1:length(cell_ddis)
    ...
    try
        ...
        testDDILOO_topology_level_parallel([folder3did ddiName], 'FisherM0', 'MATLAB');
