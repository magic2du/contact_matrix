function    [SimilarityVector, recallVector, specificityVector, precisionVector, accuracyVector, F1Vector, MCCVector, scoreVector]=predictContactsTopologyLevel_singleDDI_2010(ddi_folder,ddi_str_array)
...
for i = 1:length(ddi_str_array)
    ...
    %try
	load([ddi_folder '/iphmmA.mat']);
    load([ddi_folder '/iphmmB.mat']);
    typeofMatchA=[];
    typeofMatchB=[];
    % get predicted vecotr A
    [score, alignment, path, typeofMatchA, flags] = ...
    ...
    %get preditec vector B
        [score, alignment, path, typeofMatchB, flags] = ...
